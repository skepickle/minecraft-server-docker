#!/bin/bash

if [[ -e /.dockerenv ]]; then
  IN_DOCKER='true'
else
  IN_DOCKER='false'
fi

if [[ "${IN_DOCKER}" == "false" ]]; then tmux pipe-pane -o "cat >>$(pwd)/tmux.log"; fi

# NOTE: This script should be executed with `pwd` at the Minecraft world's base folder
. .mcenv
USE_ORACLE_JAVA=${I_ACCEPT_ORACLE_JAVA_LICENSE:-'false'}
USE_MINECRAFT=${I_ACCEPT_MINECRAFT_EULA:-'false'}


if [[ "${IN_DOCKER}" == "true" ]]; then
  USER_ID=${LOCAL_USER_ID:-9001}
  export HOME=/mc
  export DATA=/mc_data
else
  export DATA=.
  export HOME=.
fi

if [[ "${IN_DOCKER}" == "true" ]]; then
  # Create account to run Minecraft Server
  echo "Starting with UID : ${USER_ID}"
  useradd -m -d /mc -o -c "Minecraft Server" mc -s /bin/bash -u "${USER_ID}"
  chown -R mc:mc ${HOME}
  chown -R mc:mc ${DATA}
fi

#TODO: Ensure these apt packages are installed: ca-certificates wget libterm-readkey-perl libterm-readline-gnu-perl
#      and java??? of the right version?

# Accept EULA ;-)
if [[ ! -e ${DATA}/eula.txt ]]; then
  if [ "${USE_MINECRAFT}" == "true" ]; then
    echo "eula=TRUE" > ${DATA}/eula.txt
    if [[ "${IN_DOCKER}" == "true" ]]; then chown mc:mc ${DATA}/eula.txt; fi
  else
    echo "The Minecraft EULA must be accepted by passing in I_ACCEPT_MINECRAFT_EULA='true' as an environment variable"
    exit 1
  fi
fi
if [[ "${IN_DOCKER}" == "true" ]]; then
  ln -s ${DATA}/eula.txt ${HOME}/eula.txt
  chown -h mc:mc ${HOME}/eula.txt
fi

# Make sure JSON files are in place
JSON_FILES="banned-ips banned-players ops usercache whitelist"
for FILE in ${JSON_FILES}; do
  if [[ ! -e ${DATA}/${FILE}.json ]]; then
    echo -n "[]" > ${DATA}/${FILE}.json
    if [[ "${IN_DOCKER}" == "true" ]]; then chown mc:mc ${DATA}/${FILE}.json; fi
  fi
  if [[ "${IN_DOCKER}" == "true" ]]; then
    ln -s ${DATA}/${FILE}.json ${HOME}/${FILE}.json
    chown -h mc:mc ${HOME}/${FILE}.json
  fi
done

# Make sure Server Properties are set
if [[ ! -e ${DATA}/server.properties ]]; then
  cp $(dirname $0)/server.properties.example ${DATA}/
  if [[ "${IN_DOCKER}" == "true" ]]; then chown mc:mc ${DATA}/server.properties; fi
fi
if [[ "${IN_DOCKER}" == "true" ]]; then
  ln -s ${DATA}/server.properties ${HOME}/server.properties
  chown -h mc:mc ${HOME}/server.properties
fi

# Determine world folder
LEVELNAME=`cat server.properties | grep "^level-name=" | sed -e "s/^.*=//"`

# Get Server Jar
if [[ ! -e ${DATA}/minecraft_server.jar.url ]]; then
  wget -q -O - https://minecraft.net/en/download/server \
   | grep 'minecraft_server\.[0-9]\+\.[0-9]\+\.[0-9]\+\.jar' \
   | sed -e 's/^.*a href="\(.*\)">.*$/\1/' \
   > ${DATA}/minecraft_server.jar.url
  if [[ "${IN_DOCKER}" == "true" ]]; then chown mc:mc ${DATA}/minecraft_server.jar.url; fi
fi
if [[ "${IN_DOCKER}" == "true" ]]; then
  ln -s ${DATA}/minecraft_server.jar.url ${HOME}/minecraft_server.jar.url
  chown -h mc:mc ${HOME}/minecraft_server.jar.url
fi

wget -q -O - $(cat ${DATA}/minecraft_server.jar.url) > ${HOME}/minecraft_server.jar
if [[ "${IN_DOCKER}" == "true" ]]; then chown mc:mc ${HOME}/minecraft_server.jar; fi

if [[ "${IN_DOCKER}" == "true" ]]; then
  #TODO: Possibly replace this with logic to switch between openjdk alternative installs
  if [ "${USE_ORACLE_JAVA}" == "true" ]; then
    echo "deb http://ppa.launchpad.net/webupd8team/java/ubuntu xenial main" \
      > /etc/apt/sources.list.d/webupd8team-ubuntu-java-xenial.list
    apt-key adv --keyserver keyserver.ubuntu.com --recv-keys C2518248EEA14886
    echo debconf shared/accepted-oracle-license-v1-1 select true | debconf-set-selections
    echo debconf shared/accepted-oracle-license-v1-1 seen true   | debconf-set-selections
    DEBIAN_FRONTEND=noninteractive set -x \
      && apt-get update \
      && apt-get install -y --no-install-recommends \
           oracle-java8-installer
  else
    DEBIAN_FRONTEND=noninteractive set -x \
      && apt-get update \
      && apt-get install -y --no-install-recommends \
           openjdk-21-jre
  fi
fi

#EXPERIMENT!
if [[ ! -e ${DATA}/${LEVELNAME} ]]; then
  mkdir ${DATA}/${LEVELNAME}
  if [[ "${IN_DOCKER}" == "true" ]]; then chown mc:mc ${DATA}/${LEVELNAME}; fi
fi
if [[ ! -e ${DATA}/logs ]]; then
  mkdir ${DATA}/logs
  if [[ "${IN_DOCKER}" == "true" ]]; then chown mc:mc ${DATA}/logs; fi
fi

EXEC=exec
DIRECTORIES="${LEVELNAME} logs"
if [[ "${IN_DOCKER}" == "true" ]]; then
  for DIRECTORY in ${DIRECTORIES}; do
    if [[ -e ${DATA}/${DIRECTORY} ]]; then
      ln -s ${DATA}/${DIRECTORY} ${HOME}/${DIRECTORY}
      chown -h mc:mc ${HOME}/${DIRECTORY}
    else
      EXEC=
    fi
  done

  if [[ -z $@ ]]; then
    cd /mc
    echo "${EXEC} /usr/local/bin/gosu mc /tmp/wrapper.pl"
    ${EXEC} /usr/local/bin/gosu mc /tmp/wrapper.pl
  else
    echo "${EXEC} $@"
    ${EXEC} $@
  fi

  # The following is only needed in case there are new subfolders created in the container's data folder
  for DIRECTORY in ${DIRECTORIES}; do
    if [[ ! -e ${DATA}/${DIRECTORY} ]] && [[ -e ${HOME}/${DIRECTORY} ]]; then
      cp -r ${HOME}/${DIRECTORY} ${DATA}/${DIRECTORY}
      chown -R mc:mc ${DATA}/${DIRECTORY}
    fi
  done
else
  echo "${EXEC} $(dirname $0)/wrapper.pl"
  ${EXEC} $(dirname $0)/wrapper.pl
fi

